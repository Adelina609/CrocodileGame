...
public class cPainter implements Serializable {
	...
	private JFrame frame = new JFrame("Collaborative Painter (client)");
	...
	private JPanel sizeDotPanel = new JPanel();
	private JPanel sizeSliderPanel = new JPanel();
	private JSlider sizeSlider = new JSlider(JSlider.VERTICAL,10,20,10);
	private JPanel canvasPanel = new JPanel();
	private Canvas canvas = new Canvas();
	private JScrollPane canvasScrollPane;
	private ArrayList<Point> point = new ArrayList<>();
	private ArrayList<line> lines = new ArrayList<>();
	private boolean isZoomIn=false,isZoomOut=false,isServer=false;
	int strokeSize,y,x;
	private line tempLine,currentLine= new line(new ArrayList<Point>(),Color.black,sizeSlider.getValue(),1.0,1.0);
	private Dot dot;
	private Color color = new Color(red.getValue(),green.getValue(),blue.getValue());
	private double cWidth=1.0,cHeight=1.0;
...
//	transient ObjectInputStream mesOIS;
//	transient ObjectOutputStream mesOOS;
//	transient Socket mesSocket;
	...
	cPainter(Socket sock, Socket mes){
		this.sock = sock;
		clientSocket = mes;
	}
	...
	private void createScrollPane() {
		...
		this.canvasScrollPane.setMinimumSize(new Dimension(800,600));
	}
	...
	private void save(File choosenFile) throws FileNotFoundException, IOException {
	...
	void initialize(){
		...
		jtaTextAreaMessage.setFont(new Font("serif", Font.PLAIN, 18 ));
		canvasScrollPane.setMinimumSize(new Dimension(1000, 800));
	...
	class IncomingReader implements Runnable {
		...
		public void run() {
			try {
				if (OIS.readObject().getClass() == line.class) {
				while(true) {
					int i = 0;
